...
class ESC50ArtifactData(torch.utils.data.Dataset):
    def __init__(self, data_dir):
        data_dir = data_dir if data_dir.endswith('/') else data_dir + '/'
        self.annotations = pd.read_csv(data_dir + "meta/esc50_artifact.csv")
    ...
    def get_labels(self):
        """Return integer IDs for classification head size"""
        return list(self.artifact2id.values())
    
    def __getitem__(self, idx):
        if torch.is_tensor(idx): idx = idx.tolist()
        
        row = self.annotations.iloc[idx]
        audio_file_path = self.audio_dir + row['filename']
        label = self.artifact2id[row['artifact_label']]
        ...
        return {
            ...
            "data": wav,
            "sr": sr,
            "label": label,  # integer for training
